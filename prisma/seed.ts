...
// Example PDF buffer used to seed the fileData column on every document.
// This is a minimal valid single-page PDF generated by scripts/gen-example-pdf.mjs.
const EXAMPLE_PDF = Buffer.from(
...
async function main() {
  ...
  await prisma.document.createMany({
    data: [
      { propertyId: prop1.id, documentName: "חוזה שכירות — רחוב האלון 4A",    documentType: "rental_contract",   uploadedAt: new Date("2025-03-01"), fileData: EXAMPLE_PDF },
      { propertyId: prop1.id, documentName: "פרוטוקול כניסה — רחוב האלון 4A", documentType: "inventory",         uploadedAt: new Date("2025-03-01"), fileData: EXAMPLE_PDF },
      { propertyId: prop1.id, documentName: "מסירת מפתחות — רחוב האלון 4A",   documentType: "keys_record",       uploadedAt: new Date("2025-03-01"), fileData: EXAMPLE_PDF },
      { propertyId: prop2.id, documentName: "חוזה שכירות — שדרות האורן",       documentType: "rental_contract",   uploadedAt: new Date("2024-08-01"), fileData: EXAMPLE_PDF },
      { propertyId: prop3.id, documentName: "חוזה שכירות — רחוב השוק 202",    documentType: "rental_contract",   uploadedAt: new Date("2025-01-01"), fileData: EXAMPLE_PDF },
      { propertyId: prop3.id, documentName: "קבלת פיקדון — רחוב השוק",        documentType: "deposit_document",  uploadedAt: new Date("2025-01-01"), fileData: EXAMPLE_PDF },
      { propertyId: prop5.id, documentName: "חוזה שכירות — מרפסת הים יח׳ 8", documentType: "rental_contract",   uploadedAt: new Date("2025-06-01"), fileData: EXAMPLE_PDF },
      { propertyId: prop6.id, documentName: "חוזה שכירות — שדרות המשלחת",     documentType: "rental_contract",   uploadedAt: new Date("2024-12-01"), fileData: EXAMPLE_PDF },
    ],
  });
